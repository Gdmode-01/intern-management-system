    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'radial-gradient(ellipse at top left, #451a03 0%, #070b14 70%, #030712 100%)', color: '#ffffff', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <aside className="glass-card" style={{ width: '270px', borderRadius: '0 20px 20px 0', padding: '28px 20px', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '36px', paddingLeft: '8px' }}>
          <div style={{ background: 'linear-gradient(135deg, #d97706 0%, #b45309 100%)', padding: '10px', borderRadius: '12px', display: 'flex' }}>
            <span style={{ fontSize: '18px' }}>⚙️</span>
          </div>
          <div>
            <h3 style={{ margin: 0, fontSize: '15px', fontWeight: '800', color: '#ffffff' }}>Operations</h3>
      ...
      <main style={{ flex: 1, padding: '36px 44px', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <h1 style={{ fontSize: '26px', fontWeight: '800', margin: 0, color: '#ffffff' }}>Operations Command Dashboard</h1>
            ...
            <p style={{ color: '#94a3b8', fontSize: '13px', margin: '4px 0 0 0' }}>Enterprise headcount, active sprint oversight, and cohort metrics.</p>
          </div>
        ...
        {activeTab === 'overview' && (
          <div className="glass-card" style={{ borderRadius: '18px', padding: '28px' }}>
            <h3 style={{ margin: '0 0 16px 0', fontSize: '18px', fontWeight: '700', color: '#ffffff' }}>Executive Cohort Health & Department Distribution</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginTop: '16px' }}>
              <div style={{ background: 'rgba(15,23,42,0.6)', padding: '18px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
                <h4 style={{ margin: '0 0 6px 0', color: '#38bdf8' }}>Engineering & Frontend</h4>
                <p style={{ margin: 0, fontSize: '13px', color: '#94a3b8' }}>{interns.filter(i => i.department?.toLowerCase().includes('frontend') || i.department?.toLowerCase().includes('engineering')).length} Active Interns Mapped</p>
              </div>
              <div style={{ background: 'rgba(15,23,42,0.6)', padding: '18px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
                <h4 style={{ margin: '0 0 6px 0', color: '#a5b4fc' }}>Cloud Infrastructure</h4>
                <p style={{ margin: 0, fontSize: '13px', color: '#94a3b8' }}>{interns.filter(i => i.department?.toLowerCase().includes('cloud')).length} Active Interns Mapped</p>
              </div>
              <div style={{ background: 'rgba(15,23,42,0.6)', padding: '18px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
                <h4 style={{ margin: '0 0 6px 0', color: '#6ee7b7' }}>Average RM Allocation</h4>
                <p style={{ margin: 0, fontSize: '13px', color: '#94a3b8' }}>{(interns.length / (managers.length || 1)).toFixed(1)} Interns per RM</p>
              </div>
            </div>
          </div>
        )}

        {/* SECTION 2: ORGANIZATION AUDIT ROSTER */}
        {activeTab === 'roster' && (
          <div className="glass-card" style={{ borderRadius: '18px', padding: '26px' }}>
            <h3 style={{ margin: '0 0 18px 0', fontSize: '17px', fontWeight: '700', color: '#ffffff' }}>System-Wide User Audit ({users.length} Records)</h3>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', color: '#64748b', fontSize: '12px', textTransform: 'uppercase' }}>
                  ...
                  <th style={{ padding: '14px 12px' }}>Department / Mapping</th>
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '13px' }}>
                    ...
                    <td style={{ padding: '14px 12px', fontWeight: '700', color: '#ffffff' }}>{u.full_name}</td>
                    ...
                    <td style={{ padding: '14px 12px', color: '#38bdf8', fontWeight: '600' }}>
                      {u.rm_name ? `👔 ${u.rm_name}` : (u.department || 'N/A')}
                    </td>
                    <td style={{ padding: '14px 12px' }}>
                      <span style={{ padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', background: u.internship_status === 'completed' ? 'rgba(79,70,229,0.2)' : 'rgba(5,150,105,0.2)', color: u.internship_status === 'completed' ? '#a5b4fc' : '#6ee7b7' }}>
